...
public class SpeleologistAgent extends Agent {
    ...
    private class EnvironmentWanderingBehaviour extends Behaviour {
        ...
        private void makeRequestToNavigator() {
            ...
            // Log what is being sent to the Navigator
            System.out.println(getAID().getLocalName() + ": Sending request to Navigator: " + message);
        ...
        private void getNavigatorResponse() {
            ...
            if (reply2 != null) {
                if (reply2.getPerformative() == ACLMessage.PROPOSE) {
                    ...
                    System.out.println(getAID().getLocalName() + ": Received response from Navigator: " + message);
                    step++;
                }
            } else {
                block();
